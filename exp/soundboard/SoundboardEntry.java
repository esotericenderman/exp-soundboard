 ...
 public class SoundboardEntry
 {
   ...
   public boolean matchesPressed(ArrayList<Integer> pressedKeys)
   {
     ...
     if (keysRemaining == 0)
       return false;
     int[] arrayOfInt;
     int j = (arrayOfInt = this.activationKeysNumbers).length; for (int i = 0; i < j; i++) { int actkey = arrayOfInt[i];
       for (Iterator localIterator = pressedKeys.iterator(); localIterator.hasNext();) { int presskey = ((Integer)localIterator.next()).intValue();
         if (actkey == presskey) {
           keysRemaining--;
         }
       }
   ...
   public String getActivationKeysAsReadableString()
   {
     ...
     if (this.activationKeysNumbers.length == 0)
       return s;
     int[] arrayOfInt;
     int j = (arrayOfInt = getActivationKeys()).length; for (int i = 0; i < j; i++) { int i = arrayOfInt[i];
