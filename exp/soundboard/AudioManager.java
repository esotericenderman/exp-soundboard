 ...
 public class AudioManager
 {
   ...
   void playSoundClip(File file, boolean halfspeed)
   {
     ...
     if ((file.exists()) && (file.canRead())) {
       ...
       try {
         ...
         JOptionPane.showMessageDialog(null, "Selected Output Line: Primary Speaker is currently unavailable.", "Line Unavailable Exception", 0);
       }
       if ((this.secondaryOutput != null) && (this.useSecondary)) {
         try {
           ...
           JOptionPane.showMessageDialog(null, "Selected Output Line: Secondary Speaker is currently unavailable.", "Line Unavailable Exception", 0);
         }
       }
   ...
   public synchronized void setPrimaryOutputMixer(String mixerName)
   {
     ...
     String[] arrayOfString1; int j = (arrayOfString1 = mixers).length; for (int i = 0; i < j; i++) { String x = arrayOfString1[i];
       Mixer.Info[] arrayOfInfo; int m = (arrayOfInfo = AudioSystem.getMixerInfo()).length; for (int k = 0; k < m; k++) { Mixer.Info mixerInfo = arrayOfInfo[k];
